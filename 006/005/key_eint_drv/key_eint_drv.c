...
static struct class *key_eint_class;
static struct class_device	*key_eint_drv_class;
static int major = 0;
...
/* 初始化（注册）驱动 */
static int __init key_eint_drv_init(void)
{
	
	major = register_chrdev(0, "key_eint_drv", &key_eint_drv_fo);

	key_eint_class = class_create(THIS_MODULE, "key_eint_drv");
	
	key_eint_drv_class = class_device_create(key_eint_class, NULL, MKDEV(major, 0), NULL, "key_eint_drv"); /* /dev/key_eint_drv */
...
/* 卸载驱动 */
static void __exit key_eint_drv_exit(void)
{
	printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
	class_device_unregister(key_eint_drv_class);
	class_destroy(key_eint_class);
	unregister_chrdev(major, "key_eint_drv");
	iounmap(pGPFCON);
	iounmap(pGPGCON);
}
...
module_init(key_eint_drv_init);
module_exit(key_eint_drv_exit);
