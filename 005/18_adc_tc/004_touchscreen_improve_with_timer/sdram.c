...
void sdram_init()
{
	...
	**MRSRB6：0x4800002C	//Mode register set register bank6** 
	...
	MRSRB6 = 0x20
	*/
	BWSCON = 0x2000000;			//设置宽度和等待状态的寄存器
	BANKCON6 = 0x18001;			//BANK6控制寄存器
	REFRESH = 0x8404F5;			//SDRAM refresh control register
	BANKSIZE = 0xb1;			//Flexible bank size register
	MRSRB6 = 0x20;				//Mode register set register bank6
}

int sdram_test()
{

	volatile unsigned char *p = (volatile unsigned char	*)0x30000000; //初始化一个指向sdram基地址的指针p	
	int i;
	...
	/* 再读sdram看有没有写进去，能读出来对了就说明sdram启动了 */
	for (i=0; i<1000; i++)
